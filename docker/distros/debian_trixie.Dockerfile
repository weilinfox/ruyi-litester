...
RUN apt-get update && apt-get upgprade -y -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" && apt-get install -y llvm-19-tools coreutils util-linux yq file expect git make tar jq build-essential zstd && apt-get clean
