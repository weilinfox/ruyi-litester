...
RUN apt-get update && apt-get upgrade -y -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" && apt-get install -y llvm-14-tools coreutils util-linux yq file expect git make tar jq build-essential wget zstd python3-pip && apt-get clean
RUN pip install yq
